...
import { formatKeyText, type HeaderStyles, type Hint, renderSweepLine, renderWelcome, SWEEP_FRAMES } from "./header.ts";
...
describe("key formatting", () => {
	test("joins chords and alternates like pi", () => {
		assert.equal(formatKeyText(["ctrl+c"], "linux"), "ctrl+c");
		assert.equal(formatKeyText(["ctrl+p", "ctrl+o"], "linux"), "ctrl+p/ctrl+o");
	});

	test("shows alt as option on darwin", () => {
		assert.equal(formatKeyText(["alt+enter"], "darwin"), "option+enter");
	});
});
...
describe("welcome header", () => {
	test("collapsed shows logo row and one hint row", () => {
		const lines = renderWelcome(info, compact, expanded, false, 80, identityStyles).map(stripVTControlCharacters);
	...
	test("drops the hint row on narrow widths", () => {
		const narrow = renderWelcome(info, compact, expanded, false, 20, identityStyles).map(stripVTControlCharacters);
		assert.equal(narrow.length, 2);
		assert.equal(narrow[1], "pi v1.2.3");
	});

	test("expanded lists every hint on its own row", () => {
		const lines = renderWelcome(info, compact, expanded, true, 80, identityStyles).map(stripVTControlCharacters);
	...
	test("skips hints with unbound keys", () => {
		...
		const lines = renderWelcome(info, hints, hints, true, 80, identityStyles);
